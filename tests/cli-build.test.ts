...
type ReadFile = (path: string | URL, options: { encoding: string }) => Promise<string>;
...
runTest("dist cli build embeds resolveOutputFormat logic", { timeout: 60_000 }, async () => {
  const { readFile } = (await dynamicImport("node:fs/promises")) as { readFile: ReadFile };
  ...
  assert.ok(
    /const jsonOption = typeof args\.json === "string" \? args\.json : undefined;/.test(cliOutput),
  );
  assert.ok(
    /if \(jsonOption === "compact" \|\| jsonOption === "pretty"\) {\s+if \(prettyFlag\) {\s+return "pretty";\s+}\s+return jsonOption;\s+}/s.test(
      cliOutput,
    ),
  );

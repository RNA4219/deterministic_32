...
type LocalSymbolRegistryEntry = { holder: LocalSymbolHolder } | LocalSymbolWeakEntry;
...
const LOCAL_SYMBOL_OBJECT_REGISTRY: Map<
  symbol,
...
const LOCAL_SYMBOL_OBJECT_REGISTRY_FINALIZER =
  HAS_WEAK_REFS && HAS_FINALIZATION_REGISTRY
    ? new FinalizationRegistry<symbol>((symbol) => {
...
function getOrCreateSymbolObject(symbol: symbol): SymbolObject {
  ...
  if (existing !== undefined) {
    if (isWeakRegistryEntry(existing)) {
      ...
      if (LOCAL_SYMBOL_OBJECT_REGISTRY_FINALIZER !== undefined) {
        LOCAL_SYMBOL_OBJECT_REGISTRY_FINALIZER.unregister(
          existing.unregisterToken,
        );
      }
      LOCAL_SYMBOL_OBJECT_REGISTRY.delete(symbol);
    } else {
  ...
  if (LOCAL_SYMBOL_OBJECT_REGISTRY_FINALIZER !== undefined) {
    ...
    LOCAL_SYMBOL_OBJECT_REGISTRY_FINALIZER.register(
      created,
      symbol,
      unregisterToken,
    );
  } else {
    const placeholderHolder: LocalSymbolHolder = { symbol, target: created };

...
test("stableStringify and Cat32 assign distinguish Map typed array key from sentinel string key", () => {
    const cat = new Cat32();
    ...
    assert.ok(stableStringify(mapWithTypedKey) !==
...
test("stableStringify and Cat32 assign distinguish Map ArrayBuffer key from sentinel string key", () => {
    const cat = new Cat32();
    ...
    const mapWithSentinelKey = new Map([[sentinelKey, 1]]);
    assert.ok(stableStringify(mapWithBufferKey) !==
        stableStringify(mapWithSentinelKey));
...
test("stableStringify and Cat32 assign distinguish Map SharedArrayBuffer key from sentinel string key", () => {
    if (typeof SharedArrayBuffer !== "function") {
        assert.ok(true, "SharedArrayBuffer unavailable in this runtime");
        return;
    }
    ...
    const mapWithSentinelKey = new Map([[sentinelKey, 1]]);
    assert.ok(stableStringify(mapWithSharedKey) !==
        stableStringify(mapWithSentinelKey));
...
test("Cat32 assign distinguishes Map typed array key collision with sentinel string", () => {
    const cat = new Cat32();
    const first = new Uint8Array([12, 13]);
    const second = new Uint8Array([12, 13]);
    const mapWithTypedKeys = new Map([
        [first, 1],
        [second, 2],
    ]);
    ...
    const mapWithCollisionString = new Map([
        [first, 1],
        [collisionString, 2],
    ]);

...
const runBuild = async (execFile, repoRootPath, env, args = []) => await new Promise((resolve, reject) => {
    execFile(nodeExecutable, ["scripts/build.js", ...args], { cwd: repoRootPath, env }, (error, stdout, stderr) => {
        if (error != null) {
            const rejectionError = typeof error === "object" && error !== null ? error : { error };
            reject(Object.assign(rejectionError, { stdout, stderr }));
            return;
        }
        resolve();
    });

...
function parseLocalSymbolSentinel(serialized) {
    const raw = serialized.startsWith("\"") && serialized.endsWith("\"")
    ...
    const [kind, identifier, description] = payload;
...
test("stableStringify serializes identical local symbol twice with weak refs", () => {
    if (!HAS_WEAKREF || !HAS_FINALIZATION_REGISTRY)
        return;
...
test("stableStringify serializes symbols nested in sets", () => {
    ...
    const sentinelString = sentinel;
    assert.ok(sentinelString.startsWith(SET_SENTINEL_PREFIX));
    assert.ok(sentinelString.endsWith(SENTINEL_SUFFIX));
    const payloadJson = sentinelString.slice(SET_SENTINEL_PREFIX.length, -SENTINEL_SUFFIX.length);
    const entries = JSON.parse(payloadJson);
    assert.ok(Array.isArray(entries));
    const [entry] = entries;
    assert.equal(typeof entry, "string");
    const { description: parsedDescription } = parseLocalSymbolSentinel(entry);

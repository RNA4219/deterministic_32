...
const loadResolveDestination = async (token) => {
    ...
    try {
        const moduleExports = (await import(`${runnerUrl.href}?${token}=${Date.now()}`));
...
test("prepareRunnerOptions throws when destination option is missing value", async () => {
...
test("prepareRunnerOptions throws when destination option ends argv without value", async () => {
...
test("prepareRunnerOptions maps .mts target to dist .mjs path", async () => {
    ...
    const result = prepareRunnerOptions([
        ...
        existsSync: (candidate) => typeof candidate === "string" &&
...
test("prepareRunnerOptions maps .cts target to dist .cjs path", async () => {
...
for (const { extension, distExtension } of [
    ...
    test(`prepareRunnerOptions maps ./ prefixed ${extension} target to dist ${distExtension} path`, async () => {
        const prepareRunnerOptions = await loadPrepareRunnerOptions(`${extension.slice(1)}-prefixed-target`);
        const result = prepareRunnerOptions([
            "node",
            "script.mjs",
...
test("prepareRunnerOptions forwards skip pattern arguments as passthrough", async () => {
    const prepareRunnerOptions = await loadPrepareRunnerOptions("skip-pattern-passthrough");
    const result = prepareRunnerOptions([
        "node",
        "script.mjs",
        "--test-skip-pattern",
...
test("JSON reporter runner uses dist target when invoked with TS input", async () => {
    ...
    fsModule.existsSync = (candidate) => typeof candidate === "string" && knownPaths.has(candidate);
    cleanups.push(() => {
        fsModule.existsSync =
            originalExistsSync;
    });
    ...
    const destinationArgs = args.filter((value) => value.startsWith("--test-reporter-destination="));
    assert.equal(destinationArgs.length, 1);
    const resolvedDestination = `--test-reporter-destination=${pathModule.resolve("tmp/out.jsonl")}`;
...
test("JSON reporter runner maps default targets from nested directory to dist", async () => {
    const { createRequire } = (await dynamicImport("node:module"));
    const require = createRequire(import.meta.url);
    const fsModule = require("node:fs");
    const pathModule = require("node:path");
    ...
    const processWithCwd = process;
    const originalCwd = processWithCwd.cwd();
    const testsDirectory = fileURLToPath(new URL("./tests/", repoRootUrl));
    const temporaryDirectory = fsModule.mkdtempSync(pathModule.join(testsDirectory, "json-runner-"));
...
test("JSON reporter runner resolves destination from project root when invoked in subdirectory", async () => {
    const { createRequire } = (await dynamicImport("node:module"));
    const require = createRequire(import.meta.url);
    const fsModule = require("node:fs");
    const pathModule = require("node:path");
    ...
    assert.equal(expectedDefaultDestination, pathModule.resolve(projectRoot, "logs/test.jsonl"));
    const expectedOverrideDestination = activeResolveDestination("tmp/out.jsonl");
    assert.equal(expectedOverrideDestination, pathModule.resolve(projectRoot, "tmp/out.jsonl"));
    const cleanups = [];
    const mkdirCalls = [];
    const spawnCalls = [];
    const exitCodes = [];
    ...
    assert.ok(mkdirCalls.includes(expectedDirectory));
    const targetArgs = args.filter((value) => typeof value === "string" && !value.startsWith("--") && value !== "--test");
    const hasAbsoluteTargets = targetArgs.length > 0 && targetArgs.every((value) => pathModule.isAbsolute(value));
    const options = invocation.options;
    const usesProjectRootCwd = options !== null &&
        typeof options === "object" &&
        typeof options.cwd === "string" &&
        pathModule.resolve(options.cwd) === pathModule.resolve(projectRoot);
    assert.ok(hasAbsoluteTargets || usesProjectRootCwd, "runner should spawn with absolute targets or project root cwd");
    assert.deepEqual(exitCodes, [0]);
});
test("JSON reporter runner uses CLI override destination from project root when invoked in subdirectory", async () => {
    const { createRequire } = (await dynamicImport("node:module"));
    const require = createRequire(import.meta.url);
    const fsModule = require("node:fs");
    const pathModule = require("node:path");
    const { fileURLToPath } = (await dynamicImport("node:url"));
    const projectRoot = fileURLToPath(repoRootUrl);
    const activeResolveDestination = await loadResolveDestination("cli-destination");
    const expectedOverrideDestination = activeResolveDestination("tmp/cli-out.jsonl");
    assert.equal(expectedOverrideDestination, pathModule.resolve(projectRoot, "tmp/cli-out.jsonl"));
    const cleanups = [];
    const mkdirCalls = [];
    const spawnCalls = [];
    const exitCodes = [];
...
test("JSON reporter runner resolves TS targets when invoked from tests directory", async () => {
    ...
    const require = createRequire(import.meta.url);
    const fsModule = require("node:fs");
    const pathModuleForCategorizer = require("node:path");
    const cleanups = [];
    const spawnCalls = [];
    const exitCodes = [];
    let thrown;
...
test("JSON reporter runner maps directory targets into dist", async () => {
    ...
    const require = createRequire(import.meta.url);
    const fsModule = require("node:fs");
    const pathModuleForDirectory = require("node:path");
    const cleanups = [];
    const spawnCalls = [];
    const exitCodes = [];
    let thrown;
    ...
    const originalExistsSync = fsModule.existsSync;
    fsModule.existsSync = (candidate) => typeof candidate === "string" &&
...
test("prepareRunnerOptions prefers CLI targets when present", async () => {
    ...
    processWithEnv.env.__CAT32_SKIP_JSON_REPORTER_RUN__ = "1";
    try {
        const moduleExports = (await import(`${runnerUrl.href}?options=${Date.now()}`));
        ...
        const result = prepareRunnerOptions(["node", "script", "dist/tests/json-reporter.test.js"], {
        ...
        const destinationResult = prepareRunnerOptions(["node", "script", "--test-reporter-destination", "tmp/out.jsonl"], { defaultTargets: [] });
...
test("prepareRunnerOptions keeps default dist targets when invoked from frontend directory", async () => {
    const processWithEnv = process;
    ...
    const originalCwd = processWithCwd.cwd();
    try {
        const moduleExports = (await import(`${runnerUrl.href}?frontend-default=${Date.now()}`));
        const { prepareRunnerOptions } = moduleExports;
        assert.equal(typeof prepareRunnerOptions, "function");
        const { fileURLToPath } = (await dynamicImport("node:url"));
        const { resolve } = (await dynamicImport("node:path"));
...
test("prepareRunnerOptions keeps default dist targets when existsSync only reports dist entries from frontend directory", async () => {
    const processWithEnv = process;
    const previousEnv = processWithEnv.env.__CAT32_SKIP_JSON_REPORTER_RUN__;
    processWithEnv.env.__CAT32_SKIP_JSON_REPORTER_RUN__ = "1";
    const processWithCwd = process;
    const originalCwd = processWithCwd.cwd();
    try {
        const moduleExports = (await import(`${runnerUrl.href}?frontend-relative=${Date.now()}`));
        const { prepareRunnerOptions } = moduleExports;
        assert.equal(typeof prepareRunnerOptions, "function");
        const { fileURLToPath } = (await dynamicImport("node:url"));
        const frontendDirectory = fileURLToPath(new URL("./frontend/", repoRootUrl));
        ...
        const result = prepareRunnerOptions(["node", "script"], {
            existsSync: (candidate) => typeof candidate === "string" && relativeDistEntries.has(candidate),
        });
        assert.deepEqual(result.targets, ["dist/tests", "dist/frontend/tests"]);
    }
    finally {
